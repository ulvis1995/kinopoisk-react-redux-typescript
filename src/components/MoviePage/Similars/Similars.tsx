...
import { SilimarItem } from '../../types';
import { SimilarProps } from '../../typesProps';

const Similars: React.FC<SimilarProps> = ({id}) => {  
  const {setMovieId} = useActions ()
  const [similarShow, setShowAw] = useState<boolean>(false);
  const [similars, setSimilar] = useState<SilimarItem[]> ([]);
